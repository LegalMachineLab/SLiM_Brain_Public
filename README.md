...
An LLM agent (Claude Code) runs the entire pipeline. By project decision the system is **prompt-only**: apart from PDF conversion there is no code — every safeguard is a fixed procedure the agent must execute and may not alter. The agent's instructions are split so that only what must always hold is always loaded:
...
`CLAUDE_ORIGINAL.md` is the superseded v1 monolith these files grew out of, kept for provenance; the tree above is the sole normative specification.

## Design principles

- **A claim is what a paper argues.** Its central contention and the propositions it sets out to demonstrate — nothing else enters the claim record. Reported positions, supporting arguments, and definitions are deliberately not claims.
...
| Piece | State |
|---|---|
| Specification (schema + skills) | ✅ complete, `schema_version: 3.2` |
| PDF → markdown conversion | ✅ implemented and run (10 sources converted) |
| Write gate |([schema/write-gate.md](schema/write-gate.md)) |
...
| Interactive viewer | ✅ generated `viewer.html` + shared artifact (`publish-viewer` skill) |
| Corpus | 10 sources, re-extracted under 3.2 (129 claims) |
...
Record versioning, run ids, and what counts as a schema change — and therefore forces re-extraction of the corpus — are defined in [schema/SCHEMA.md](schema/SCHEMA.md).
